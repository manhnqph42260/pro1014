<?php
// views/login.php – LOGIN PAGE DUY NHẤT
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($role === 'admin') {
            ...
            // Login cho HDV
            $stmt = $conn->prepare("SELECT * FROM guides WHERE (email = ? OR guide_code = ?) AND status = 'active' LIMIT 1");
            $stmt->execute([$username, $username]);
            $user = $stmt->fetch();
