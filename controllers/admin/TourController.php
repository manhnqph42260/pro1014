...
class TourController extends BaseController { // THÊM "extends BaseController"
    ...
    private function handleCreateTour() {
        ...
        try {
            ...
            $errors = $this->validateRequired(['tour_code', 'tour_name', 'departure_date'], $_POST);
            if (!empty($errors)) {
                $this->setFlash('error', implode("<br>", $errors));
                return;
            }
            
            // INSERT TOUR - chỉ insert column có sẵn trong database
    ...
    private function handleImageUpload($tour_id) {
        ...
        if (!empty($_FILES['featured_image']['name'])) {
            if ($_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
                $file_name = uniqid() . '_featured_' . basename($_FILES['featured_image']['name']);
